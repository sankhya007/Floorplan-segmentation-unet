# this is the code that will scan the floorplan using the unet model for now we have the best results from this one 
...
# this segment is for the RESTnet model itself but direct connection is not possible so will be training this model tomorrow jst to see if there is any good parsing potencial that this type of learning can give to this project 
...
# # TTA: run inference with flips and average — free improvement, no retraining needed
...
#     # TTA variants: original + 3 flips
#     # each patch is run 4 times with different orientations and averaged
#     tta_variants = [
#         lambda x: x,                                      # original
#         lambda x: np.flip(x, axis=0).copy(),              # vertical flip
#         lambda x: np.flip(x, axis=1).copy(),              # horizontal flip
#         lambda x: np.flip(np.flip(x, axis=0), axis=1).copy(),  # both flips
#     ]
#     tta_inverses = [
#         lambda x: x,
...
#     print(f"Running inference on {total_patches} patches x {len(tta_variants)} TTA variants...")
...
#             for aug, inv in zip(tta_variants, tta_inverses):
#                 aug_patch  = aug(patch)
#                 tensor     = preprocess_patch(aug_patch, device)
#                 with torch.no_grad():
#                     pred = model(tensor)
#                 pred = torch.sigmoid(pred).squeeze().cpu().numpy()
#                 pred = np.clip(pred, 0.05, 0.95)
...
#     # Otsu thresholding — picks threshold automatically per image
#     # better than fixed 0.5 when floorplan brightness differs from training distribution
